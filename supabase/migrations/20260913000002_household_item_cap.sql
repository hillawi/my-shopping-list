-- Hard safety cap: at most 500 distinct items per household, regardless of plan. This is
...
-- it updates the existing one. This cap is really "500 distinct item names ever used by a
-- household", not "500 purchases" — expected to comfortably cover a real household's active
-- list plus purchase history.
--
...
begin
  if not exists (
    ...
    if (select count(*) from shopping_items where household_id = new.household_id) >= 500 then
